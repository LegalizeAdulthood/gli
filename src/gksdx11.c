...
#define WindowName "GLIgks V4.5.30"
...
typedef struct ws_state_list_struct {
    ...
    unsigned long pixels[MAX_COLORS], ccolor;
...
void pixmap_to_rf (void)
{
    ...
    if (pix != NULL && beimage != NULL)
        ...
        write (p->rf, rmap, MAX_COLORS);
        write (p->rf, gmap, MAX_COLORS);
        write (p->rf, bmap, MAX_COLORS);
...
void pixmap_to_uil (void)
{
    ...
    image = XGetImage (p->dpy, p->pixmap, 0, 0, p->width, p->height,
        AllPlanes, ZPixmap);
    n = MAX_COLORS < PRIVATE_COLORS ? MAX_COLORS : PRIVATE_COLORS;
