...
C*  DIGITAL LN03 PLUS
  ...
  204         CONTINUE
  207         CONTINUE
   38         CALL GKDTK1 (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
              GOTO 4000

C*  TEKtronix 41xx Series
   82         CALL GKDTK2 (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
              GOTO 4000

C*  HP-GL
   51         CONTINUE
   53         CALL GKDHP (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
              GOTO 4000

C*  VT3xx
   16         CONTINUE
   17         CALL GKDVT3 (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
              GOTO 4000
...
C*  PostScript, Display PostScript, LJ250
   ...
   92         IPTR(1) = SOPWS(I,5)
              IPTR(2) = SOPWS(I,6)
              CALL GKDPS (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,
     *          LC,CHARS,IPTR(1))
              GOTO 4000
...
C*  Adobe's Portable Document Format (PDF)
  ...
  102         IPTR(1) = SOPWS(I,5)
              IPTR(2) = SOPWS(I,6)
              CALL GKDPDF (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,
     *          LC,CHARS,IPTR(1))
              GOTO 4000
...
C*  GKSM Output Metafile
    2         IPTR(1) = SOPWS(I,5)
              IPTR(2) = SOPWS(I,6)
              CALL GKDMFO (FID,DX,DY,DIMX,IA(1),LRX,RX(1),LRY,RY(1),
     *          LC,CHARS,IPTR(1))
              GOTO 4000
...
C*  GKSM Input Metafile
    3         IPTR(1) = SOPWS(I,5)
              IPTR(2) = SOPWS(I,6)
              CALL GKDMFI (FID,DX,DY,DIMX,IA(1),LRX,RX(1),LRY,RY(1),
     *          LC,CHARS,IPTR(1))
              GOTO 4000
...
C*  CGM (Computer Graphics Metafile)
    ...
    8         IPTR(1) = SOPWS(I,5)
              IPTR(2) = SOPWS(I,6)
              CALL GKDCGM (FID,DX,DY,DIMX,IA(1),LRX,RX(1),LRY,RY(1),
     *          LC,CHARS,IPTR(1))
              GOTO 4000

C*  WISS (Workstation Independent Segment Storage)
    5         CALL GKDWIS (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
              GOTO 4000

C*  IBM PC w\ VGA
C*  VAX UIS
   41         CALL GKDGA (FID,DX,DY,DIMX,IA(1),LRX,RX(1),LRY,RY(1),
     *          LC,CHARS)
              GOTO 4000

C*  PBM (Portable BitMap)
  103         CONTINUE
  104         CALL GKDBM (FID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
              GOTO 4000
...
C               select workstation
        ...
        SUBROUTINE GKDDLS (FCTID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
C               devive driver link routine (needed for gks.f to work
C               around a bug in NAG Fortran 95)

        INTEGER FCTID, DX, DY, DIMX, IA(*), LRX, LRY
        REAL RX(*), RY(*)
        CHARACTER*(*) CHARS

        CALL GKDDLK (FCTID,DX,DY,DIMX,IA,LRX,RX,LRY,RY,LC,CHARS)
